...
# IDENTIFICACIÓN ESTRUCTURAL-SVAR
def generar_celda_bexo(vars_: list):
    """
    Imprime una celda lista para copiar y modificar manualmente,
    con el formato de bexo adaptado a las variables actuales.
    """
    n = len(vars_)
    header = "     # " + "  ".join(f"{v:<6}" for v in vars_)
    print("# Block exogeneity (var in column j does not affect var in row i)")
    print(header)
    print("bexo = {")
    for i, var in enumerate(vars_):
        fila = ", ".join(["0"] * n)
        coma = "," if i < n - 1 else ""
        print(f"    '{var}': [{fila}]{coma}  # equation {var}")
    print("}")
    print()
    print("# Cambia 0 → 1 donde quieras restringir (excluir) una variable.")


def parsear_bexo(bexo: dict, vars_: list) -> np.ndarray:
    """
    Convierte el dict bexo definido manualmente a np.ndarray.
    """
    n = len(vars_)
    mat = np.zeros((n, n), dtype=int)
    for i, var in enumerate(vars_):
        if var not in bexo:
            raise KeyError(f"Falta la ecuación '{var}' en bexo.")
        fila = bexo[var]
        if len(fila) != n:
            raise ValueError(f"Ecuación '{var}' tiene {len(fila)} valores, se esperan {n}.")
        mat[i] = fila
    print(" bexo:")
    print(pd.DataFrame(mat, index=vars_, columns=vars_).to_string())
    return mat

def bexo_a_restricciones(bexo: np.ndarray) -> np.ndarray:
    """
    Convierte bexo a matriz A para statsmodels SVAR.
    nan = libre | 0 = restringido | 1 = diagonal
    """
    A = np.where(bexo == 1, 0.0, np.nan)
    np.fill_diagonal(A, 1.0)
    return A


def estimar_svar(model, bexo: np.ndarray, vars_: list):
    """
    Estima SVAR con restricciones de corto plazo definidas en bexo.
    """
    from statsmodels.tsa.api import SVAR
    A = bexo_a_restricciones(bexo)

    print("\n Matriz A de restricciones:")
    df_A = pd.DataFrame(
        np.where(np.isnan(A), "?", A.astype(int).astype(str)),
        index=vars_, columns=vars_
    )
    print(df_A.to_string())
    print("  ? = libre  |  0 = restringido  |  1 = diagonal")

    svar_model   = SVAR(model.endog, svar_type="A", A=A)
    svar_results = svar_model.fit(maxiter=1000)

    print(f"\n SVAR estimado  |  llf = {svar_results.llf:.4f}")
    print("\n  Matriz A estimada:")
    print(pd.DataFrame(svar_results.A, index=vars_, columns=vars_).round(6).to_string())

    return svar_results, A

# IRF — FUNCIONES IMPULSO-RESPUESTA

def calcular_irf(results, steps: int = 20, orth: bool = True):
    """
    Calcula las funciones impulso-respuesta.

    Parámetros
    ----------
    results : VARResultsWrapper
    steps   : horizonte de impulso-respuesta
    orth    : True = IRF estructurales ortogonalizadas (Cholesky)

    Retorna el objeto IRAnalysis de statsmodels.
    """
    return results.irf(steps)


def plot_irf(results, vars_: list, steps: int = 20,
             orth: bool = True, figsize: tuple = (13, 9)):
    """
    Grafica las IRF ortogonalizadas (o no) con bandas de confianza.
    Usa el plot nativo de statsmodels pero con título personalizado.
    """
    irf = calcular_irf(results, steps=steps, orth=orth)
    tipo = "Estructurales — Cholesky" if orth else "Reducidas"
    fig = irf.plot(orth=orth, figsize=figsize)
    plt.suptitle(f"Funciones Impulso-Respuesta ({tipo})",
...
def plot_irf_custom(results, vars_: list, steps: int = 20,
                    orth: bool = True, figsize: tuple = (13, 9)):
    """
    IRF con gráficos personalizados (sin usar el plot de statsmodels).
    Muestra solo las respuestas de cada variable a cada choque.
    """
    irf     = calcular_irf(results, steps=steps, orth=orth)
    n       = len(vars_)
    h       = np.arange(steps + 1)

    # Extraer arrays: irf.irfs[paso, respuesta, choque]
    if orth:
        irfs = irf.orth_irfs
    else:
        irfs = irf.irfs
    ...
    fig.suptitle(f"IRF — {tipo}", fontsize=13, fontweight="bold")
    ...
    for i, resp in enumerate(vars_):
        for j, shock in enumerate(vars_):
            ax = axes[i][j]
            y  = irfs[:, i, j]
            ax.plot(h, y, color=colors[j % len(colors)], linewidth=1.6)
            ax.axhline(0, color="gray", linewidth=0.8, linestyle="--")
            ax.set_title(f"{shock} → {resp}", fontsize=8)
            if i == n - 1:
                ax.set_xlabel("Horizonte", fontsize=7)
