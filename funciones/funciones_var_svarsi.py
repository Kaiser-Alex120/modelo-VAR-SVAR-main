...
def plot_irf_signos(results, vars_: list, signos: dict,
                    ...
                    figsize: tuple = (13, 11)):
    """
    IRF personalizada con control de signos por celda.

    Parámetros
    ----------
    results   : VARResultsWrapper
    vars_     : lista de variables
    signos    : dict  {variable_respuesta: [s1, s2, s3, s4]}
                  1  = mostrar tal cual
                 -1  = invertir signo
                  0  = ocultar (celda en gris)
    steps     : horizonte
    orth      : True = Cholesky
    ci        : True = bandas de confianza bootstrap
    alpha_ci  : nivel para las bandas (0.16 ≈ ±1σ, 0.05 ≈ 95%)
    ...
    if ci:
        try:
            irf_ci = results.irf(steps)
            lower  = irf_ci.cum_effect_lower if orth else irf_ci.effect_lower
            upper  = irf_ci.cum_effect_upper if orth else irf_ci.effect_upper
            # Usar el método correcto según versión de statsmodels
            bands  = irf.errband_mc(orth=orth, svar=False,
                                    repl=500, signif=alpha_ci)
            lower, upper = bands
            tiene_ci = True
        except Exception:
            tiene_ci = False
    ...
    fig.suptitle(f"Funciones Impulso-Respuesta — {tipo}  (signos personalizados)",
                 fontsize=13, fontweight="bold")

    for i, resp in enumerate(vars_):
        fila_signos = signos.get(resp, [1] * n)

        for j, shock in enumerate(vars_):
            ax  = axes[i][j]
            s   = fila_signos[j]        # signo para esta celda
            ...
            if s == 0:
                ...
                ax.set_title(f"{shock} → {resp}", fontsize=8, color="#AAAAAA")
                continue

            # ── Valores IRF con signo aplicado ────────────────────────────
            y = irfs[:, i, j] * s
            ...
            if tiene_ci:
                y_lo = lower[:, i, j] * s
                y_hi = upper[:, i, j] * s
                if s == -1:                  # al invertir, lo y hi se intercambian
                    y_lo, y_hi = y_hi, y_lo
                ax.fill_between(h, y_lo, y_hi,
                                color=colors[j % len(colors)],
                                alpha=0.15, label=f"IC {int((1-alpha_ci)*100)}%")

            ax.plot(h, y, color=colors[j % len(colors)],
                    linewidth=1.8, label=f"{shock} → {resp}")
            ax.axhline(0, color="gray", linewidth=0.8, linestyle="--")

            # Etiqueta de signo invertido
            if s == -1:
